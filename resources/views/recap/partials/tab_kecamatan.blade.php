...
<div x-show="activeFilter === 'kecamatan'" x-transition class="tab-kecamatan grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-5">
    @foreach ($kecamatanDataList as $kecName => $kecData)
        <a href="{{ route('recap.kecamatan.show', $kecName) }}" data-search-key="{{ strtolower($kecName) }}" x-show="search === '' || '{{ strtolower($kecName) }}'.includes(search.toLowerCase())" class="group block bg-gradient-to-br from-indigo-50/50 to-white border border-indigo-100 rounded-2xl p-6 shadow-sm hover:border-indigo-300 hover:shadow-md transition-all duration-300 relative overflow-hidden">
            <div class="absolute right-0 top-0 w-32 h-32 bg-indigo-500 opacity-5 rounded-bl-full pointer-events-none group-hover:scale-110 transition-transform duration-500"></div>
            <div class="flex justify-between items-start mb-2 relative z-10">
                <div>
                    <span class="text-indigo-600 text-[10px] font-black uppercase tracking-widest bg-indigo-100/80 px-2 py-1 rounded mb-3 inline-block">Kecamatan</span>
                    <h4 class="font-extrabold text-slate-800 text-xl group-hover:text-indigo-700 transition-colors duration-200 truncate pr-2">
                        {{ $kecName }}
                    </h4>
                </div>
                <div class="text-right flex-shrink-0">
                    <span class="block text-2xl font-black text-indigo-900 leading-none">{{ number_format($kecData['total_kasus']) }}</span>
                    <span class="text-[9px] text-slate-500 font-bold uppercase tracking-wide mt-1 block">Akumulasi Kasus</span>
                </div>
            </div>
            ...
            <div class="mt-5 pt-4 border-t border-indigo-100/50 flex justify-between items-center relative z-10 w-full group-hover:border-indigo-200 transition-colors">
                <span class="text-xs text-slate-500 font-medium">Lihat rincian penuh</span>
                <span class="bg-indigo-600 text-white rounded-lg p-1.5 shadow-sm group-hover:scale-110 transition-transform">
